...
# 메소드 : 클래스 내부에 정의된 함수로, 인스턴스의 데이터를 조작하거나 동작을 정의한다.
    ...
    #       객체를 통해 접근시 호출되는 메소드의 첫 번쨰 인자로 항상 self를 넘겨 주어야한다.
...
# 네임스페이스는 크게 다섯가지로 나누어 볼수 있다.
# 1. 지역 네임스페이스 : 현재 함수나 메소드 내의 네임 스페이스
# 2. 인스턴스 네임스페이스 : 인스턴스 개체의 네임스페이스
# 3. 클래스 네임스페이스 : 클래스 객체의 네임스페이스
# 4. 전역 네임스페이스 : 모듈(py파일) 내의 전역 네임스페이스
...
def modify_global():
      global g_variable
      g_variable = "global modfien in function"
...
# 비공개 속성에 직접 접근 시도
# __(언더스코어 2개)로 시작하는 속성은 완전한 의미의 private 는 아니지만, 어느 정도 보회되는 속성으로 취급함.
# 이름 이름 장식(name manging) 이라고 함.
# 속성의 이름을 내부적으로 변걍하여 클래스 외부에서 직접 접근하는 것을 어렵게 만듦.
# print(my_account.__balance) 오류가 발생한다.
...
class Student2(Leaner1):
      def __init__(self,name,subject):
            Leaner1.__init__(self,subject)
            self.name = name
